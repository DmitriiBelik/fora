...
const Adress: React.FC = () => {
    ...
    const token: string = 'eabcbd6cfd6e07ea9a378327c8051217cf47302d'
    const [query, setQuery] = useState<string>('')
    const [adresses, setAdresses] = useState<IElement>()
    ...
    const getAdresses = () => {
        if (query.length >= 3) {
            fetch(url, options)
                ...
                .then((result) => setAdresses(result['suggestions']))
    ...
    return (
        <div className='page-wrapper'>
            <h2 className='page-title'>Поиск адресов</h2>
            <p className='page-text'>Введите интересующий вас адрес</p>
            ...
            {adresses ? (
                <div className='adress-wrapper'>
                    ...
                    {adresses.map(
                        (element: {
                            data: { fias_id: Key | null | undefined }
                            value:
                                | string
                                | number
                                | boolean
                            ...
                            return <p key={element.data.fias_id}>{element.value}</p>
                        },
                    )}
                </div>
            ) : (
